...
def ticker_download(index_url, ticker_column_name, yahoo_suffix, table_idx, start_date, end_date):
...
def index_download(ticker, start_date, end_date):
    prices_list = []
    prices = dr.DataReader(ticker,'yahoo',start_date,end_date)['Adj Close']
    prices = pd.DataFrame(prices)
    prices.columns = [ticker]
...
def test_corr(x,y):
    spear = x.corrwith(y) # Spearman Correlation
    print(f'The Linear Correlation Coefficient for the two returns is {spear[0]:.2f}.');
    
    Tau = stats.kendalltau(x,y) #Check (Kendall) rank correlation with simple returns
    print(f'The Ordinal Assosciation Kendall test for the two returns is {Tau[0]:.2f}.');

    
def table_info(x):
    avg_ret = (x.mean()*252)*100
    std = (x.std()*sqrt(252))*100
    sh = (avg_ret/std)*100
...
def dist_plot(x,y,label):
    fig, ax = plt.subplots(figsize=(16,8));
    ax.set_title(label);
    sns.distplot(x, bins=50, color='hotpink', label='Conditional on Clusters Portfolio'); #Consolidated Cluster Returns
    sns.distplot(y, bins=50, color='blue', label = 'Purely Random Portfolio'); #Unconsolidated random returns (without replacement)
    plt. legend();
